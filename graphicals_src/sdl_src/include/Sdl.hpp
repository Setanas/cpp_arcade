...
*/
	...
	public:
		...
		void	displayGame(const ent_list &dent, map_str &map,
			int gamesLibIt);
	private:
		void	checkAlphanumericKey(Keys_inp_t &key, int keyCode);
		void	checkSpecialKey(Keys_inp_t &key, int keyCode, bool all);
		...
		bool	fillTextureVector(std::vector<sf::Texture> &texture,
			const std::string &path);
		void	displayEntities(const ent_list &ent, int gameLibIt);
		void	displayMap(map_str &map, int gameLibIt);
		void	displaySprite(float posX, float posY, int gameLibIt,
			int textureIt);
	private:
